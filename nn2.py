...
class Predictor:
    def __init__(self, column, lr=0.001, n_epochs=100, batch_size=10):
        self.column = column
        self.lr = lr
    ...
    def train(self, X, y):
        ...
        for epoch in range(self.n_epochs):
            for i in range(0, len(X), self.batch_size):
                ...
                ybatch = y[i:i+self.batch_size] / meta_data.loc[self.column, "total_space"]
    ...
    def predict(self, X):
        ...
        with torch.no_grad():
            y_pred = self.model(X)
        return np.array(y_pred * meta_data.loc[self.column, "total_space"], dtype=np.float32)


def train_predict(train_data, test_data, column):
    clf = Predictor(column=column)
    ...
    clf.train(poly_features_train, y_train)
